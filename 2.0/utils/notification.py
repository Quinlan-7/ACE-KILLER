...
# v2.2: windows_toasts（WinRT 通知库）改为惰性导入。
# 原因：在部分 Python 版本环境中，windows_toasts 先于 PySide6 加载会引发
# DLL 加载顺序冲突，导致进程访问冲突（0xC0000005）。
# 惰性导入确保 PySide6 先完成加载，再按需加载通知库，保证导入顺序安全。
_toasts_module = None


def _load_toasts():
    """惰性加载 windows_toasts 模块（只加载一次）"""
    global _toasts_module
    if _toasts_module is None:
        from windows_toasts import (  # noqa: F401
            InteractableWindowsToaster, Toast, WindowsToaster, ToastImagePosition,
            ToastButton, ToastDisplayImage, ToastAudio,
        )
        _toasts_module = sys.modules["windows_toasts"]
    return _toasts_module
...
def get_toaster():
    ...
    global _toaster
    if _toaster is None:
        _toasts_module = _load_toasts()
        _toaster = _toasts_module.InteractableWindowsToaster('')
    return _toaster
...
def send_notification(title, message, icon_path=None, buttons=None, silent=True):
    ...
    try:
        ...
        # 根据silent参数设置音频
        audio = toasts.ToastAudio(silent=True) if silent else toasts.ToastAudio()

        # 创建Toast对象
        toast = toasts.Toast(text_fields=[title, message], audio=audio)

        # 添加图标
        if icon_path and os.path.exists(icon_path):
            try:
                toast.AddImage(
                    toasts.ToastDisplayImage.fromPath(
                        icon_path, position=toasts.ToastImagePosition.AppLogo
                    )
                )
        ...
        if buttons:
            for button in buttons:
                if isinstance(button, dict):
                    ...
                    toast.AddAction(toasts.ToastButton(text, action, launch=launch))
                elif isinstance(button, str):
                    # 支持简单字符串格式的按钮
                    toast.AddAction(toasts.ToastButton(button, f'action={button.lower()}'))
