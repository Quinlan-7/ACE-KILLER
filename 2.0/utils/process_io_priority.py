    ...
    def _set_cpu_affinity_by_mode(self, process_id: int, performance_mode: int) -> bool:
        ...
        try:
            ...
            if affinity_strategy == "last_core":
                # v2.2: 效能模式 —— 拓扑感知的“效能核心”
                # AMD：最后一个物理核心；Intel 混合架构：能效核（E-core）
                from core.cpu_topology import get_eco_target_cpus
                eco_cores = get_eco_target_cpus()
                if not eco_cores:
                    eco_cores = [self._cpu_count - 1]
                proc.cpu_affinity(eco_cores)
                logger.debug(
                    f"成功设置进程(PID={process_id})的CPU亲和性到效能核心{eco_cores}"
                )
            else:  # "all_cores"
                # 其他模式：绑定到所有核心
...
def set_process_priority_with_fallback(pid: int, priority_config: dict) -> tuple:
    """
    设置进程优先级，带多层降级策略

    Args:
        pid: 进程 ID
        priority_config: 优先级配置字典，支持:
            - io_priority: int (0-3)
            - cpu_priority: str (IDLE/BELOW_NORMAL/NORMAL/HIGH/REALTIME)
            - max_cores: int
            - power_throttling: bool

    Returns:
        (bool, str): (是否完全成功, 使用的方法描述)
    """
    methods_used = []
    all_success = True

    pm = get_io_priority_manager()

    # 尝试设置 IO 优先级
    if "io_priority" in priority_config:
        try:
            success = pm.set_process_io_priority(pid, priority_config["io_priority"])
            if success:
                methods_used.append("NtSetInformationProcess(IO)")
            else:
                logger.warning(f"PID:{pid} NtSetInformationProcess 失败，尝试 SetPriorityClass")
                # 降级: 使用 SetPriorityClass 设置 CPU 优先级替代
                try:
                    proc = psutil.Process(pid)
                    proc.nice(psutil.IDLE_PRIORITY_CLASS)
                    methods_used.append("SetPriorityClass(fallback)")
                    success = True
                except Exception as e2:
                    logger.warning(f"PID:{pid} SetPriorityClass 也失败: {e2}")
                    all_success = False
        except Exception as e:
            logger.warning(f"PID:{pid} IO 优先级设置异常: {e}")
            all_success = False

    # 尝试设置 CPU 优先级
    if "cpu_priority" in priority_config:
        try:
            priority_map = {
            ...
            win_priority = priority_map.get(
                priority_config["cpu_priority"], IDLE_PRIORITY_CLASS
            )
            proc = psutil.Process(pid)
            proc.nice(win_priority)
            methods_used.append(f"SetPriorityClass({priority_config['cpu_priority']})")
        except Exception as e:
            logger.warning(f"PID:{pid} CPU 优先级设置失败: {e}")
            all_success = False

    # 尝试设置 CPU 亲和性
    if "max_cores" in priority_config:
        try:
            proc = psutil.Process(pid)
            total = psutil.cpu_count()
            max_c = min(priority_config["max_cores"], total)
            affinity = list(range(total - max_c, total))
            proc.cpu_affinity(affinity)
            methods_used.append(f"CPU_Affinity({max_c}cores)")
        except Exception as e:
            logger.warning(f"PID:{pid} CPU 亲和性设置失败: {e}")
            all_success = False

    # 尝试设置效能节流
    if priority_config.get("power_throttling"):
        try:
            from ctypes import wintypes
            ...
            if handle:
                ...
                ntdll.NtSetInformationProcess(
                    handle, PROCESS_POWER_THROTTLING_INFORMATION,
                    ctypes.byref(state), ctypes.sizeof(state)
                )
                kernel32.CloseHandle(handle)
                methods_used.append("PowerThrottling")
        except Exception:
            pass

    method_str = "+".join(methods_used) if methods_used else "none"
    return (all_success, method_str)
...
class PerformanceConfigApplier:
    ...
    def apply(self, pid: int, config: dict) -> tuple:
        """
        应用性能配置到进程

        Args:
            pid: 进程 ID
            config: 配置字典，包含 io_priority, cpu_priority, max_cores, power_throttling

        Returns:
            (bool, str): (是否完全成功, 使用的方法)
        """
        return set_process_priority_with_fallback(pid, config) 