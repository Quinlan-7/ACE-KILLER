...
from core.win32_constants import (
    ...
    POWER_THROTTLING_PROCESS_ENABLE,
    POWER_THROTTLING_PROCESS_DISABLE,
    PROCESS_POWER_THROTTLING_STATE,
)

# v2.2: ACE 反作弊相关进程（供游戏模式等模块引用）
ACE_PROCESSES = ["ACE-Tray.exe", "SGuard64.exe", "AntiCheatExpert.exe"]
...
class GameProcessMonitor:
    ...
    def __init__(self, config_manager):
        ...
        self._cache_lock = threading.Lock()  # v2.1: 进程缓存线程锁
    ...
    def refresh_process_cache(self, force=False):
        ...
        if not (force or (current_time - self.last_cache_refresh) >= self.cache_timeout):
            return
    ...
    def set_process_priority_and_affinity(self, process_name):
        ...
        if proc:
            try:
                ...
                logger.debug(
                    f"优化进程: {process_name}，已设置为效能模式，绑定核心: {eco_cores}"
                )
                return True
            except Exception as e:
        ...
        if process_name.lower() == self.scanprocess_name.lower():
            try:
                ...
                try:
                    ...
                    # v2.2: 期望的效能核心（拓扑感知）
                    expected_core = get_eco_target_cpus() or []
                    
                    # 判断CPU亲和性是否符合优化要求
                    if expected_core:
                        # 只要设置了亲和性，或者亲和性包含了期望的效能核心，就认为是优化了
                        if len(cpu_affinity) == 1 or any(
                            core in cpu_affinity for core in expected_core
                        ):
    ...
    def monitor_acetray_process(self):
        ...
        if not kill_enabled:
            ...
            while self.running:
                ...
                if ace_proc:
                    # 转为效能模式优化（不终止）
                    self.set_process_priority_and_affinity(self.anticheat_name)
    ...
    def start_monitors(self):
        ...
        # v2.0: 启动磁盘 IO 统计
        self.disk_stats.start_monitoring()

        # v2.0: 尝试 WMI 模式
        if self.use_wmi:
            self._start_wmi_monitor()
    ...
    def _start_wmi_monitor(self):
        """尝试启动 WMI 进程监控"""
        try:
            from utils.wmi_monitor import WmiProcessMonitor
            self.wmi_monitor = WmiProcessMonitor()

            if not self.wmi_monitor.available:
                logger.info("WMI 不可用，使用轮询模式")
                self.wmi_monitor = None
                return

            # 注册进程创建回调 - 自动应用规则
            self.wmi_monitor.on_process_created(self._on_wmi_process_created)
            self.wmi_monitor.start()
            logger.success("WMI 进程监控已启动")
        except Exception as e:
            logger.warning(f"WMI 启动失败，使用轮询模式: {e}")
            self.wmi_monitor = None
